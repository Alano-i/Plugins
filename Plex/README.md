...
- Tautulli 需要添加 `yaml` `googletrans` 支持（进入tautulli命令行，执行以下命令，或在 tautulli 的 `start.sh` 中加入自动安装的模块）（翻译需要网络环境支持,如果网络环境不支持或不想翻译，将 `translate_switch` 设置为 `off`,默认为`on`，如果网络环境不支持还设置为 `on`，将会报错 ）
```console
pip3 install pyyaml
```
```console
