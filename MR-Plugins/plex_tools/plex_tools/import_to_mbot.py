...
def push_sub(videos, media_type='Movie'):
    loger.info(f"{plugins_name}正在查找本地媒体，请稍候...")
        ...
        if media_type=='show':
            for s in video:
                ...
                if response.status_code == 200:
                    loger.info(f"{plugins_name}['{title}'] {i+1}/{video_len} 导入（提交订阅）成功")
                else:
                    loger.error(f"{plugins_name}['{title}'] {i+1}/{video_len} 导入（提交订阅）失败")
        else:
...
def push_sub_main(lib_name):
    ...
    try:
        ...
        loger.error(f"{plugins_name}获取媒体出现错误! 原因：{e}")