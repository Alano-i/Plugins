...
def audio_clip_m_echo(ctx: PluginCommandContext,
                ...
                audio_start: ArgSchema(ArgType.String, '剪辑开始时间', '默认：00:00:29', default_value='00:00:29', required=False),
                audio_end: ArgSchema(ArgType.String, '剪辑结束倒数秒数', '默认：12，单位：秒', default_value='12', required=False),
                clip_configs: ArgSchema(ArgType.Enum, '选择运行的操作，默认：剪辑、整理、添加元数据', '若只剪辑，下方不填', enum_values=lambda: clip_config, default_value='clip_and_move', multi_value=False, required=False),
